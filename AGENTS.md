...
## Overview
Follow C++ Core Guidelines with emphasis on consistency, modular design, immutability, and minimal mutation.

## Philosophy
Prioritize consistency over familiarity. Embrace modern C++ for safety and maintainability.
...
- **Default values:** use `{}` instead of explicit defaults like `nullptr`, `0`, etc.
...
- **Friend declarations:** omit template parameter names: `template<typename> friend class foo;`
...
- **Rule of Five:** define all or none of: copy ctor/move ctor/dtor/copy assign/move assign
- **Rule of Zero:** prefer classes without custom special members
- **Copy-and-swap:** MUST use copy-and-swap pattern consistently; define copy and move assignment in terms of `friend swap` function
...
- **Logical operators:** **MUST** use keyword forms `not`, `and`, `or` instead of symbols `!`, `&&`, `||`
